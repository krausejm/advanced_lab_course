...
for file in file:
    ...
    for i in range(len(y)):
        chisq+=((y[i]-ychi[i]))**2#/(y_error[i])**2
    print(y)
    print(ychi)
    print(y_error)
    cov_err=pcov[0][1]
    ...
    print("chi2=",chisq)
    if "_x" in file:
        plt.plot(x_new,y_new, label="$x(\\alpha_x)=(%.4f\pm%.4f)\\frac{mm}{mrad}\cdot \\alpha + (%.4f\pm%.4f)$mm\n$\\chi^2=%.4f$"%(a,a_err,b,b_err, chisq))
    else:
        plt.plot(x_new,y_new, label="$z(\\alpha_z)=(%.4f\pm%.4f)\\frac{mm}{mrad}\cdot \\alpha + (%.4f\pm%.4f)$mm\n$\\chi^2=%.4f$"%(a,a_err,b,b_err, chisq))
    #plt.text(0,-47,"$\chi^2$={:.4f}".format(chisq))
    counter += 1
plt.legend(bbox_to_anchor=(1,1))
plt.xlabel("$\\alpha_{x,z}$ / mrad", fontsize=20)
plt.ylabel("$x, z$ / mm", fontsize=20)
plt.savefig("../../TeX/figs/calibration/"+"kick_verification.pdf", bbox_inches='tight')
#plt.show()
